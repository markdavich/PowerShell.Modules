...
function Get-ViewMapForAllViews {
    ...
    [PSCustomObject] $result = @{
        Keys = Get-ViewKeys -Folder $FolderPath -Recurse:$Recurse
        Map  = @{ }
    }
    ...
    foreach ($view in $views) {
        $result.Map[$view.Name] = Get-ViewMapForSingleView $view
    }
...
function Get-CodeBehindPath {
    ...
    if (Test-CodeBehindExists $View $CodeBehindFileName) {
        return [FileSystemInfo](Get-Item -Path (Get-UntestedCodeBehindPath $View $CodeBehindFileName))
    }
...
function Get-ViewKeys {
    param (
        [FileSystemInfo] $Folder,
        [switch] $Recurse = $false
    )

    [FileSystemInfo[]] $views = Get-ViewFileList -Folder $Folder -Recurse:$Recurse

    [string[]] $result = $views.Name
